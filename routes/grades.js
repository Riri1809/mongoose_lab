...
router.delete('/:id/scores/:scoreId', async (req, res) => {
    try {
      ...
      const scoreIndex = grade.scores.findIndex((score) => score._id == req.params.scoreId);
  
      // check if the score exists
      if (scoreIndex === -1) return res.status(404).send('Score not found!');
  
      // remove the score from the array
      grade.scores.splice(scoreIndex, 1);
